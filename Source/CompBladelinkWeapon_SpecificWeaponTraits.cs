...
namespace MyWeapons
{
    public class CompBladelinkWeapon_SpecificWeaponTraits : CompBladelinkWeapon
    {
        ...
        public new List<WeaponTraitDef> TraitsListForReading => traits;

        public new CompProperties_BladelinkWeapon_SpecificWeaponTraits Props => (CompProperties_BladelinkWeapon_SpecificWeaponTraits)props;

        public new int TicksSinceLastKill
        {
            get
            {
                if (lastKillTick < 0)
        ...
        private void InitializeTraits()
        {
            ...
            Log.Warning(new string('-', 20));

            List<string> pre_defined_traits = Props.personas;
            for (int i = 0; i < pre_defined_traits.Count; ++i)
            {
